...
public class Restaurant : IDisposable, IRestaurant
{
    ...
    {
        Console.WriteLine($"Добрый день. Подождите секунду, я подберу столик и подтвержу бронь. Вам придет {_notificationService.Name}");
        Task.Run(async () =>
        {
            ...
            {
                _logger.LogError(ex, "Ошибка при бронировании столика на {0}", seatsCount);
                message = $"Сервис временно недоступен. Приносим свои извенения. Вас уведомим {_notificationService.Name} о решении проблемы";
            }
            finally
            {
    ...
    {
        Console.WriteLine($"Добрый день. Подождите секунду, я сниму бронь со столика {number}. Вам придет {_notificationService.Name}");
        Task.Run(async () =>
        {
            await semaphoreSlim.WaitAsync(cancel).ConfigureAwait(false);
            string message = "";
